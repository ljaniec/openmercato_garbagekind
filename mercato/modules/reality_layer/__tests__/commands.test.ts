...
describe('reality_layer.intent.create', () => {
  ...
  it('returns an identical request on idempotent replay by the same originator', async () => {
    const { ctx } = makeCtx({
      actorId: ORIGINATOR,
      existingIntent: makeIntent({ status: 'pending' }),
    })
    const result = await createPhysicalIntentCommand.execute(createInput, ctx)
  ...
  it('does not let another actor claim an idempotent replay', async () => {
    const { ctx } = makeCtx({
      actorId: SUPERVISOR,
      existingIntent: makeIntent({ status: 'pending' }),
    })
