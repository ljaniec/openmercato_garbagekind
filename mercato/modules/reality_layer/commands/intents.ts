...
> = {
  ...
  async execute(rawInput, ctx) {
    ...
    // Resolve the trusted command principal before checking idempotency. Otherwise
    // a second principal who somehow learns a client's UUID could replay the same
    // request and obtain another actor's resource as a "successful" retry.
    ...
    if (existing) {
      if (
        existing.originatorUserId !== principal.userId ||
        existing.originatorKind !== principal.kind
      ) {
        throw new Error('PhysicalIntent idempotency key belongs to a different originator.')
      }
